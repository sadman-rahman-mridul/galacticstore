...
const showProducts = (products) => {
  ...
  for (const product of allProducts) {
    ...
    div.innerHTML = `<div class="product-card single-product">
      <div>
    <img class="product-image" src=${image}></img>
      </div>
      <h3 class="product-name-color">${product.title}</h3>
      <p>Category:<span class="catagory-color"> ${product.category} </span></p>
      ...
      <p> Total Ratings: <i class="fas fa-user"></i> ${product.rating.count} </p>
      
      <h3>Price: $${product.price}</h3>
      <button onclick="addToCart(${product.id},${product.price})" id="addToCart-btn" class="btn add-cart-button">Add To Cart</button>
